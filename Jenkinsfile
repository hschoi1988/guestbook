...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="minyowoo/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/hschoi1988/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_minyowoo') {
                        oDockImage.push()
                    }
                }
            }
    ...
    post { 
        always { 
            slackSend(tokenCredentialId: 'slack-token2'
                , channel: '#education'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) 빌드가 진행되었습니다. Details: (<${BUILD_URL} | here >)")
        }
        success { 
            slackSend(tokenCredentialId: 'slack-token2'
                , channel: '#education'
